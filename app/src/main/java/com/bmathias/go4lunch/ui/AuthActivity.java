...
import static com.bmathias.go4lunch.utils.Constants.RC_SIGN_IN;
...
import com.bmathias.go4lunch.databinding.ActivityAuthBinding;
import com.bmathias.go4lunch.injection.Injection;
import com.bmathias.go4lunch.injection.ViewModelFactory;
import com.bmathias.go4lunch.viewmodel.AuthViewModel;
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == RC_SIGN_IN) {
            ...
            if (resultCode == RESULT_OK) {
               showSnackBar(getString(R.string.connection_success));
            } else {
                // ERRORS
                if (response == null) {
    ...
    private void signInWithAuthCredential(AuthCredential googleAuthCredential) {
        ...
        authViewModel.authenticatedUserLiveData.observe(this, authenticatedUser -> {
            if (authenticatedUser != null) {
                goToMainActivity();
                toastMessage(authenticatedUser.getUserName());
            }
        });
    }

    private void toastMessage(String name) {
       Toast.makeText(this, getString(R.string.login_snackbar_hi) + name
               + getString(R.string.login_snackbar_exclamation_mark) +"\n"
               + getString(R.string.login_snackbar_account_created), Toast.LENGTH_LONG).show();
    }
